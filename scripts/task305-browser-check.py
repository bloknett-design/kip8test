...
with sync_playwright() as p:
    ...
    # Task 311: пояснительные тултипы с ячеек убраны — смена по циклу
    # показывает бейдж с кодом в самой ячейке (см. тесты K выше)
    check('L: Task 311 — тултип смены 05.09 убран (title пуст)',
          tip == '', tip[:200])

    # M. Тултип плана 21.09: план отпуска БЕЗ плановой смены (Task 306)
    tipp = infop['title'] if infop and 'title' in infop else ''
    # Task 311: тултип плана убран — код «ОТ» в ячейке (класс ws-vac-plan)
    check('M: Task 311 — тултип плана 21.09 убран (title пуст)',
          tipp == '' and infop and infop['plan'], tipp[:250])
