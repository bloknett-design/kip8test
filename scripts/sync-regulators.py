#!/usr/bin/env python3
"""
Синхронизация регуляторов с публичной ссылкой Яндекс Диска.

Источник: https://disk.yandex.ru/i/KNVTQ7Q6II7zbA
          (файл «Перечень КИП ИОС рабочий.xlsx», тот же что и для приборов/блокировок/клапанов)
Лист: "Регуляторы_app"
...
(раздел «Клапаны»):
  ...
  3. Парсит лист "Регуляторы_app" — заголовки в 1-й строке, данные со 2-й.
  4. Сохраняет результат в data/regulators.json.

Переменные окружения:
  REGULATORS_PUBLIC_KEY — публичная ссылка
...
# (по образцу scripts/sync-valves.py / sync-lockouts.py)
# ============================================================
...
def main():
    public_key = os.environ.get('REGULATORS_PUBLIC_KEY', '').strip() or DEFAULT_PUBLIC_KEY
