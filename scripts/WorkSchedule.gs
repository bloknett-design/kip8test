...
//   создание листов таблицы — разовый запуск eventsInit в
//   редакторе Apps Script (или авто-создание первой записью
//   addTraining, Task 405/413)
//
...
var WorkSchedule = {
    ...
    if (lastRow >= 2) {
      ...
      for (var i = 0; i < ids.length; i++) {
        if (parseInt(ids[i][0], 10) === id) {
          ...
          // общего при любом значении листа, Task 423)
  ...
  // Task 424 (заявка: проработка блока «Мероприятия» — «в
  // мероприятиях должны выбираться из списка обучение/прогул/
  // примечание, и также вся информация должна архивироваться в
  // архиве; для этих целей нужно создать листы в файле
  // табель_КИП_ИОС»): ИНИЦИАЛИЗАЦИЯ ЛИСТОВ блока в таблице
  // табель_КИП_ИОС. Разовый запуск в редакторе Apps Script
  // (eventsInit) — через API приложения НЕ доступен (как
  // instrListInit / trainingsSplitInit). Что делает:
  //   1) создаёт лист «Инструктажи» с каноническими заголовками
  //      Task 418 (id/таб_номер/тип/тема/дата_проведения/выполнение/
  //      просрочен/комментарий), если листа нет; СУЩЕСТВУЮЩИЙ лист
  //      не трогается (формат столбцов определяет
  //      _trainingsSheetFormat по заголовкам строки 1);
  //   2) создаёт лист «Мероприятия» с каноническими заголовками
  //      (id/таб_номер/тип/тема/дата_начала/дата_окончания/
  //      длительность_дней/комментарий), если листа нет;
  //   3) переносит строки обучение/прогул/примечание из
  //      «Инструктажей» в «Мероприятия» (легаси-строки прежней
  //      объединённой таблицы — splitTrainingsSheet Task 405:
  //      id и значения сохраняются, связки Записи_графика не
  //      ломаются; формат done конвертируется: дата_окончания =
  //      дата_проведения, длительность 1 день); листа
  //      «Инструктажи» нет — перенос пропускается (не ошибка).
  // Идемпотентно: повторный запуск не трогает существующие листы
  // и не находит строк к переносу (moved: 0). АРХИВ мероприятий —
  // сам лист «Мероприятия»: записи не удаляются со временем,
  // годовые архивы блока читают их все (eventsAll, Task 408).
  eventsInit: function() {
    var ss = SpreadsheetApp.openById(this.SPREADSHEET_ID);
    var sheets = {};

    // (1) «Инструктажи» — заголовки Task 418; существующий лист
    // возвращается как есть (перезаписи заголовков НЕТ)
    var trSheet = ss.getSheetByName(this.TRAININGS_SHEET);
    sheets[this.TRAININGS_SHEET] = trSheet ? 'existing' : 'created';
    if (!trSheet) trSheet = this._ensureTrainingsSheet();

    // (2) «Мероприятия» — прежний формат A..H (мероприятия
    ...
    // (3) перенос обучение/прогул/примечание из «Инструктажей»
    // (листа нет — перенос пропускается; ошибка переноса — в
    // ответ, инициализация листов при этом уже выполнена)
    var split = null;
    if (trSheet) {
      split = this.splitTrainingsSheet();
    } else {
      split = { ok: true, moved: 0, skipped: 'no_Инструктажи' };
    }

    try {
      Utils.audit('', 'WORKSCHEDULE_EVENTS_INIT', '', '',
        'Инициализация блока «Мероприятия» (Task 424): листы — ' +
        JSON.stringify(sheets) + '; перенесено строк — ' +
        ((split && split.moved) || 0));
    } catch (e) { /* ignore */ }
    return { ok: true, sheets: sheets, moved: (split && split.moved) || 0,
             split: split };
  }
...
// ============================================================
// Task 424: инициализация листов блока «Мероприятия»
// ============================================================
// ЗАПУСК (проект Apps Script табель_КИП_ИОС — тот же, где
// WorkSchedule.gs): в выпадающем списке функций редактора выбрать
// eventsInit → ▶ Run (первый запуск может попросить авторизацию —
// разрешить). Результат — в журнале (Ctrl+Enter). Что делает:
//   1) создаёт лист «Инструктажи» с заголовками
//      (id / таб_номер / тип / тема / дата_проведения / выполнение /
//      просрочен / комментарий), если его нет; СУЩЕСТВУЮЩИЙ лист
//      не трогается;
//   2) создаёт лист «Мероприятия» с заголовками (id / таб_номер /
//      тип / тема / дата_начала / дата_окончания / длительность_дней /
//      комментарий), если его нет — это ПОСТОЯННЫЙ АРХИВ записей
//      блока: строки не удаляются со временем, годовые архивы
//      карточки работника («Мероприятия · ‹год›», стрелки листают
//      годы) читают ВСЕ записи;
//   3) переносит строки обучение/прогул/примечание из «Инструктажей»
//      в «Мероприятия» (легаси прежней объединённой таблицы; id и
//      все значения строк СОХРАНЯЮТСЯ — связки Записи_графика не
//      ломаются); листа «Инструктажи» нет — перенос пропускается.
// Повторный запуск безопасен (листы уже есть — не трогаются,
// переносить нечего — moved: 0). Тип мероприятия в приложении
// выбирается СПИСКОМ: обучение / прогул / примечание (форма «Новое
// мероприятие» из карточки работника или попапа ячейки шахматки).
function eventsInit() {
