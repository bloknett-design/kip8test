...
//   страницы, инструктажи менять не нужно, блоки не должны
//   влиять друг на друга»): создание листа — разовый запуск
//   eventsInit в редакторе Apps Script (инициализирует ТОЛЬКО
//   «Мероприятия», лист «Инструктажи» не затрагивается) или
//   авто-создание первой записью addTraining (Task 405/413);
//   автосоздания/автопродления сроков у мероприятий НЕТ —
//   вносятся вручную каждый раз (см. _autoCreateNextTrainings)
//
...
var WorkSchedule = {
  ...
  _readInstrListSheet: function() {
    ...
    // (эталон заявки: «период 3 месяца»). Task 423 (заявка
    // «9-ОГЭ создалось через 6 месяцев а не через 3»):
    // эффективная связь 9-ОГЭ — встроенный общий → период
    // ребёнка = 3 мес ЭТАЛОНА ЗАЯВКИ, значение листа («6»,
    // скопированное с родителя «(6 месяцев)») НЕ главнее:
    // выполнение общего обязано создавать 9-ОГЭ РОВНО через
    // 3 месяца (заявки 419/421); связь на ДРУГОЙ пункт листа —
    // период из листа (осознанное решение пользователя)
    try {
      ...
      if (childIdx >= 0 && parentIdx >= 0 && childIdx !== parentIdx) {
        ...
        // Task 423 (заявка «9-ОГЭ создалось через 6 месяцев а
        // не через 3»): эффективная связь 9-ОГЭ — встроенный
        // общий (столбец «в составе» пуст/опечатка/указывает на
        // него же) → период ребёнка = 3 мес ЭТАЛОНА заявки 419
        // («период 3 месяца»), значение листа НЕ главнее: живой
        // лист несёт «6» (скопировано с родителя «(6
        // месяцев)»), а выполнение общего создаёт 9-ОГЭ РОВНО
        // через 3 месяца (заявка 421). Связь на ДРУГОЙ пункт
        // листа — период из листа (не форсируем)
        if (this._sameInstrName(
              String(out[childIdx]['в составе'] || '').trim(),
              out[parentIdx].название)) {
  ...
  // Task 423 (заявка «9-ОГЭ создалось через 6 месяцев а не через
  // 3»): srvVer '423' — период зависимого ребёнка при связи с
  // общим = 3 мес эталона заявки независимо от значения листа;
  // клиент предупреждает о сервере < 423 (см. _readInstrListSheet)
  // Task 425 (заявка: «мероприятия вносятся вручную и без
  // автоматического продления, инструктажи/мероприятия не должны
  // влиять друг на друга»): тип отмечаемой строки — обучение/
  // прогул/примечание (легаси-строки в «Инструктажах») → отметка
  // фиксируется, автосоздания НЕТ (стоп-правило
  // _autoCreateNextTrainings); srvVer '425'
    ...
    if (lastRow >= 2) {
      ...
      for (var i = 0; i < ids.length; i++) {
        if (parseInt(ids[i][0], 10) === id) {
          ...
          // Task 422: srvVer — версия сервера автосоздания
          // (клиент отличает старый Apps Script: нет поля —
          // логика 419..421; '423' — период 9-ОГЭ = 3 мес от
          // общего при любом значении листа, Task 423; '425' —
          // мероприятия без автосоздания, Task 425)
  ...
  // Task 419/420/421 (заявки): автосоздание записей с новыми
  // сроками проведения при отметке выполнения (setTrainingDone,
  // 0 → 1). Периодичность N — «Список_И_и_ПЗ». Task 420:
  // соответствия тема ↔ название ↔ «в составе» НЕСТРОГИЕ
  // (норм-ключ ИЛИ сигнатура — «№»/тире/точки/регистр ручного
  // заполнения не ломают связь). Task 421 (заявка «9-ОГЭ
  // зависимый от общего»): правило ЦИКЛ «отметка ребёнка →
  // родитель» Task 420 УДАЛЕНО — выполнение РЕБЁНКА («в
  // составе», 9-ОГЭ) НЕ создаёт записей, только фиксация его
  // выполнения; новые записи ребёнка появляются при выполнении
  // РОДИТЕЛЯ (правило 2). Правила:
  //   1) ОТМЕЧЕННЫЙ пункт (N > 0) БЕЗ «в составе»: новая запись
  //      того же пункта на дата + N мес, если в окне
  //      (дата; дата + N] ещё нет записи ни самого пункта, ни
  //      его РОДИТЕЛЯ («в составе»: проведение родителя
  //      покрывает пункт — отдельная запись не нужна);
  //      пункт-РЕБЁНОК — БЕЗ собственного автосоздания (Task 421);
  ...
  //      Task 423: период ребёнка при связи с общим = 3 мес
  //      ЭТАЛОНА заявки («период 3 месяца») — значение листа
  //      («6», скопированное с родителя) не сдвигает срок
  ...
  // Task 425 (заявка: «новые инструктажи формируются автоматически
  // в зависимости от предыдущих, а мероприятия каждый раз
  // вносятся вручную и без автоматического продления, потому что
  // у них нет закономерности — фиксируют уже случившееся»):
  // СТОП-ПРАВИЛО для мероприятийных типов (обучение/прогул/
  // примечание — легаси-строки прежней объединённой таблицы могли
  // остаться в «Инструктажах») — отметка выполнения НЕ создаёт
  // записей: автосоздание — ТОЛЬКО у инструктажей, блоки независимы
  _autoCreateNextTrainings: function(sheet, rowData) {
    ...
    var provDate = rowData[4];
    // Task 425: мероприятие — без автопродления (вносится вручную)
    var tipKey = String(tip || '').trim().toLowerCase()
                   .replace(/\s+/g, '_');
    if (this.TRAINING_EVENTSHEET_TYPES[tipKey]) {
      return { created: created, updated: updated, skipped: [],
               note: 'Task 425: мероприятие (' + tip + ') — ' +
                     'автосоздания нет (вносится вручную, без продления)' };
    }
    ...
    // Task 421 (заявка «9-ОГЭ зависимый от общего»): правило (0)
    // ЦИКЛ Task 420 (отметка ребёнка → создание РОДИТЕЛЯ на
    // дата + N ребёнка) УДАЛЕНО — выполнение РЕБЁНКА не создаёт
    // записей вообще; цикл «по кругу» держится на выполнении
    // РОДИТЕЛЯ: общий создаёт и 9-ОГЭ +3 мес (правило 2), и общий
    // +6 мес (правило 1)

    // (1) новый срок отмеченного пункта — дата + N мес; Task 421:
    // пункт-РЕБЁНОК («в составе» разрешён в пункт листа) — только
    // фиксация выполнения, собственного автосоздания НЕТ (новый
    // срок ребёнка появляется при выполнении родителя — правило 2)
    if (per > 0 && !parentItem) {
  ...
  // Task 407/409: инициализация листа «Список_И_и_ПЗ» — создать
  ...
  // Task 424/425 (заявка: проработка блока «Мероприятия» —
  // «в мероприятиях должны выбираться из списка обучение/прогул/
  // примечание, и также вся информация должна архивироваться в
  // архиве; для этих целей нужно создать листы в файле
  // табель_КИП_ИОС»; Task 425: «инструктажи менять не нужно, а для
  // мероприятий нужно создать свои отдельные страницы,
  // проверить, чтобы они не влияли друг на друга — новые
  // инструктажи формируются автоматически, мероприятия вносятся
  // вручную и без автоматического продления»): ИНИЦИАЛИЗАЦИЯ
  // ЛИСТА «Мероприятия» — СВОЯ ОТДЕЛЬНАЯ страница блока в
  // таблице табель_КИП_ИОС. Разовый запуск в редакторе Apps
  // Script (eventsInit) — через API приложения НЕ доступен (как
  // instrListInit). Что делает:
  //   1) создаёт лист «Мероприятия» с каноническими заголовками
  //      (id/таб_номер/тип/тема/дата_начала/дата_окончания/
  //      длительность_дней/комментарий), если листа нет — это
  //      ПОСТОЯННЫЙ АРХИВ записей блока; СУЩЕСТВУЮЩИЙ лист
  //      не трогается;
  //   2) БОЛЬШЕ НИЧЕГО: лист «Инструктажи» НЕ создаётся и НЕ
  //      изменяется, перенос строк между листами НЕ выполняется
  //      (Task 425 — блоки независимы: маршрутизация addTraining
  //      по типу — Task 405; автосоздание сроков — только
  //      инструктажи, setTrainingDone Task 419..423)
  // Идемпотентно: повторный запуск — existing, ничего не меняет.
  // АРХИВ мероприятий — сам лист «Мероприятия»: записи не
  // удаляются со временем, годовые архивы блока читают их все
  // (eventsAll, Task 408).
  eventsInit: function() {
    var ss = SpreadsheetApp.openById(this.SPREADSHEET_ID);
    var sheets = {};

    // «Мероприятия» — прежний формат A..H (мероприятия
    // многодневны: дата_начала/дата_окончания/длительность_дней);
    // существующий лист возвращается как есть (перезаписи
    // заголовков НЕТ)
    var evSheet = ss.getSheetByName(this.EVENTS_SHEET);
    sheets[this.EVENTS_SHEET] = evSheet ? 'existing' : 'created';
    if (!evSheet) evSheet = this._ensureEventsSheet();

    try {
      Utils.audit('', 'WORKSCHEDULE_EVENTS_INIT', '', '',
        'Инициализация листа «Мероприятия» (Task 425): ' +
...
// ============================================================
// Task 424/425: инициализация листа «Мероприятия»
// ============================================================
// ЗАПУСК (проект Apps Script табель_КИП_ИОС — тот же, где
// WorkSchedule.gs): в выпадающем списке функций редактора выбрать
// eventsInit → ▶ Run (первый запуск может попросить авторизацию —
// разрешить). Результат — в журнале (Ctrl+Enter), например:
//   eventsInit: {"ok":true,"sheets":{"Мероприятия":"created"}}
// Что делает:
//   1) создаёт лист «Мероприятия» с заголовками (id / таб_номер /
//      тип / тема / дата_начала / дата_окончания / длительность_дней /
//      комментарий), если его нет, — это СВОЯ ОТДЕЛЬНАЯ страница
//      блока мероприятий и ПОСТОЯННЫЙ АРХИВ записей: строки не
//      удаляются со временем, годовые архивы карточки работника
//      («Мероприятия · ‹год›», стрелки листают годы) читают ВСЕ
//      записи; существующий лист не трогается;
//   2) НИЧЕГО БОЛЬШЕ: лист «Инструктажи» НЕ создаётся и НЕ
//      изменяется, строки НЕ переносятся (заявка Task 425:
//      инструктажи менять не нужно, блоки независимы).
// Повторный запуск безопасен (лист уже есть — не трогается).
// Тип мероприятия в приложении выбирается СПИСКОМ: обучение /
// прогул / примечание (форма «Новое мероприятие» из карточки
// работника или попапа ячейки шахматки). Мероприятия НЕ имеют
// автосоздания/автопродления — вносятся вручную каждый раз
// (заявка Task 425: не плановые, фиксируют уже случившееся);
// автосоздание новых сроков — ТОЛЬКО у инструктажей (отметка
// выполнения, Task 419..423).
function eventsInit() {
