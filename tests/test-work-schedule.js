...
describe('График работы — WorkSchedule', () => {
    ...
    describe('Индекс ключа записи (дата|таб_№)', () => {
        ...
        test('ключ формируется как ISO|таб_№', () => {
            const idx = buildIndex([
                { 'дата': '2026-08-15', 'таб_№': '2741', 'статус': 'Д' }
            ]);
            assertTrue('2026-08-15|2741' in idx);
        });
        test('последняя запись перезаписывает предыдущую с тем же ключом', () => {
            const idx = buildIndex([
                { 'дата': '2026-08-15', 'таб_№': '2741', 'статус': 'Д', _v: 1 },
                { 'дата': '2026-08-15', 'таб_№': '2741', 'статус': 'И', _v: 2 }
            ]);
            assertEqual(idx['2026-08-15|2741']['статус'], 'И');
        });
        test('разные таб_№ в один день → разные ключи', () => {
            const idx = buildIndex([
                { 'дата': '2026-08-15', 'таб_№': '2741', 'статус': 'Д' },
                { 'дата': '2026-08-15', 'таб_№': '5464', 'статус': 'Н' }
            ]);
