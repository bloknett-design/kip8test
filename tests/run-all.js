...
// Task 407 — блок инструктажей по ШАБЛОНУ «Список_И_и_ПЗ»: сервер
// (instrList/instrAll в listTrainings, чтение по заголовкам,
// instrListInit); группы шаблона + записи года + «след. срок»/«просрочено»
// + «вне списка»; datalist «Темы»; кэш
require('./test-task407.js');
// Task 408 — строгий select «Тема» из шаблона + подсказка пункта;
...
// Task 410 — форма «+ Инструктаж…»: instr-режим — выбор пункта
// «Список_И_и_ПЗ» одним списком (группы по виду) без поля «Тип»;
// мероприятие — типы обучение/прогул/примечание; тип записи = «вид»
// пункта (_instrTypeOfTheme); попап ячейки — дефолт «обучение»
require('./test-task410.js');
// Task 411 — форма инструктажа/ПЗ в картах работников: работник —
// статичная строка (список фамилий убран, значение из карточки
// входа + скрытое поле); ОДНА дата «Дата проведения» (окончание/
// длительность скрыты; submit пишет однодневную запись);
// мероприятие — прежний период
require('./test-task411.js');
// Task 412 — форма «+ Инструктаж…» всегда даёт строгий выбор пункта:
// пустой ответ сервера (Apps Script старый / лист «Список_И_и_ПЗ» не
// создан / кэш прежней версии) — встроенный эталон _INSTR_CANON
// (ровно 5 пунктов заявки 409, как instrListInit); живой лист главный
require('./test-task412.js');
// Task 413 — записи раздела не добавлялись: лист «Инструктажи» удалён
// из файла вручную → addTraining падал sheet_not_found. Автосоздание
...
// .ws-popup-name + заголовки блоков; Task 417: попап удалён)
require('./test-task415.js');
// Task 416 — столбец E «сокращение» в «Список_И_и_ПЗ»: короткие
// названия в печати табеля, окнах мероприятий и мини-окнах
// (тултип бейджа / окно дня / карточка + заголовки групп)
require('./test-task416.js');
// Task 417 — полные названия в блоках карт; попап по фамилии
// удалён полностью; editTraining — единый пул (архивные годы)
require('./test-task417.js');
// Task 418 — столбцы «дата_проведения»/«выполнение»/«просрочен»
// листа «Инструктажи»: отметка о выполнении (галочка карточки),
...
// Task 422 — «9-ОГЭ +3 не создаётся»: запись родителя в окне ребёнка
// не подавляет создание; skipped с причиной; srvVer/autoNote в ответе
require('./test-task422.js');
// Task 423 — «9-ОГЭ создалось через 6 месяцев а не через 3»: период
// зависимого ребёнка при связи с общим = 3 мес эталона заявки —
// значение листа («6» с родителя) не сдвигает срок; srvVer 423
require('./test-task423.js');
// Task 424 — блок «Мероприятия»: eventsInit — инициализация листа
// «Мероприятия» табель_КИП_ИОС (разовый запуск в редакторе
// Apps Script), архив eventsAll; соответствие блока: select
// типов обучение/прогул/примечание (Task 425: eventsInit —
// ТОЛЬКО «Мероприятия», «Инструктажи» не затрагивает)
require('./test-task424.js');
// Task 425 — независимость блоков: инструктажи (автосоздание
// сроков 419..423) и мероприятия (вручную, без автопродления)
// не влияют друг на друга; стоп-правило автосоздания для
// типов обучение/прогул/примечание; srvVer '425'
require('./test-task425.js');
