...
Task ID: 239
Agent: main
Task: Пользователь сообщил «Раздел Расходомеры хозрасчётные не отображается в боковом меню». Сделать раздел видимым в сайдбаре без необходимости раскрывать группу.

Work Log:
- Анализ структуры сайдбара (index.html, блок .sidebar-content):
  • Группы сайдбара (sidebar-group): КИП и А (6), Электротехника (1),
    Геометрия (6), Экзаменационные билеты (4, data-requires="library"),
    Библиотека КИП и А (5, data-requires="library"),
    КИП ИОС (7, data-requires="kip-ios"),
    Документация ИОС (1 пункт: «Расходомеры хозрасчётные»).
  • Все пункты внутри групп имеют CSS-класс sidebar-item-extra →
    CSS-правило `.sidebar-item-extra { display: none; }` скрывает их
    до тех пор, пока родительская группа не получит класс .expanded
    (через toggleSidebarGroup()).
  • Только 4 пункта без -extra (sidebar-item): Админ-панель, Обновить,
    Светлая тема, О приложении — они всегда видимы.

- Найдена причина: раздел «Расходомеры хозрасчётные» был единственным
  пунктом внутри сворачиваемой группы «Документация ИОС» (data-group=
  "docs-ios"). Пользователь видел заголовок группы «Документация ИОС»,
  но сам пункт «Расходомеры хозрасчётные» был скрыт CSS-классом
  sidebar-item-extra, пока пользователь не кликнет на заголовок группы,
  чтобы её развернуть. Так как в группе всего один пункт — раскрывать
  её ради доступа к нему — неудобный UX.

- Проверка логики _applyRoleToUI (строки ~27614-27682):
  • Цикл по всем .sidebar-item: проверяет, есть ли страница (из onclick
    navigateTo('xxx')) в allowed для роли. Если да — item.style.display=''
    (не 'none'). Это снимает inline-стиль, но CSS sidebar-item-extra
    {display:none} продолжает действовать, пока группа не развернута.
  • Видимость самой группы определяется наличием хотя бы одного
    видимого (style.display !== 'none') пункта внутри.
  • Роли с доступом к flowmeter-data: КИП8 pro, КИП ИОС дежурный, ИТР8,
    ИТР8 pro, ИТР ИОС, Админ. Для них группа «Документация ИОС» была
    видна, но пункт внутри — только после разворачивания.

- Дизайн решения:
  1. Удалить группу «Документация ИОС» (data-group="docs-ios") целиком
     из сайдбара — она содержала только один пункт, ради которого
     приходилось её разворачивать.
  2. Вынести «Расходомеры хозрасчётные» как top-level sidebar-item
     (без sidebar-item-extra класса → display:flex по умолчанию,
     всегда виден ролям с доступом, без необходимости разворачивать).
  3. Сохранить цвет (rgba(200,112,72,0.9) — оранжево-коричневый),
     добавить SVG-иконку gauge/тренога для визуальной согласованности
     с другими утилитарными кнопками внизу сайдбара.
  4. Видимость управляется общей логикой _applyRoleToUI — как у всех
     sidebar-item с navigateTo: role has 'flowmeter-data' → виден,
     иначе скрыт. Никаких отдельных проверок не требуется.
  5. Страница «Документация ИОС» (page-docs-ios) НЕ затронута — она
     доступна через кнопку на главной и через breadcrumb
     back-navigation как и раньше. Её кнопка «Расходомеры хозрасчётные»
     (flowmeterMenuBtn) продолжает работать (отдельная логика,
     строка ~27714).

- Файлы изменены:
  • index.html (строки ~34902-34918):
    - Удалён блок <div class="sidebar-group" data-group="docs-ios">
      с заголовком «Документация ИОС» и единственным пунктом внутри.
    - Добавлен top-level <div class="sidebar-item"
      onclick="navigateTo('flowmeter-data'); toggleSidebar();"> с
      SVG-иконкой (path d="M12 19V5" + d="M5 12l7-7 7 7" + d="M5 19h14")
      и текстом «Расходомеры хозрасчётные» в цвете rgba(200,112,72,0.9).
  • sw.js: CACHE_VERSION v502 → v503.
  • tests/test-role-access.js:
    - SIDEBAR_GROUPS: удалён ключ 'docs-ios' (группа больше не существует).
    - Тест «Каждая группа сайдбара в HTML имеет счётчик»: >= 7 → >= 6
      (групп теперь 6: kipa, electro, geometry, exam-tickets, library,
      kip-ios; docs-ios удалена).
    - Тест «Группа Документация ИОС (расходомеры): видима только ролям
      с фильтром 10» → заменён на «Группа «Документация ИОС» удалена
      (Task 239): пункт Расходомеры вынесен как top-level» — проверяет:
        (a) docs-ios группы нет в HTML,
        (b) есть новый top-level sidebar-item с navigateTo('flowmeter-data')
            и текстом «Расходомеры хозрасчётные»,
        (c) старой разметки sidebar-item-extra для flowmeter-data нет.
    - Тест «Гость: 0 доступных пунктов документации»: из списка групп
      удалён 'docs-ios'.
  • tests/test-flowmeter-validation.js:
    - Task 238 SW-блок (v502 present, v501 gone) заменён исторической
      заметкой (v502 → v503 в Task 239).
    - Добавлен describe «Task 239: top-level sidebar-item
      «Расходомеры хозрасчётные»» — 4 теста:
        1. Сайдбар: есть top-level sidebar-item для flowmeter-data.
        2. Сайдбар: старая разметка sidebar-item-extra для flowmeter-data
           убрана.
        3. Сайдбар: группа «Документация ИОС» (data-group="docs-ios")
           удалена.
        4. Сайдбар: текст «Расходомеры хозрасчётные» присутствует ровно
           1 раз как sidebar-item (без учёта кнопки на странице
           Документация ИОС, у которой другой класс — menu-btn).
    - Добавлен describe «Task 239: SW версия v503» — 2 теста:
      CACHE_VERSION = kipia-test-v503, старая v502 убрана.

- Тесты: 802 → 806 passed (+4), 0 failed.

Stage Summary:
- Task 239 выполнен. Раздел «Расходомеры хозрасчётные» теперь виден
  в сайдбаре как самостоятельный top-level пункт (между группой «КИП ИОС»
  и тонким разделителем), всегда отображается для ролей с доступом к
  flowmeter-data — без необходимости раскрывать какую-либо группу.
- Корневая причина: раздел был спрятан внутри сворачиваемой группы
  «Документация ИОС» с CSS-классом sidebar-item-extra → display:none,
  пока пользователь не кликнет на заголовок группы. В группе был всего
  один пункт, что делало раскрытие ради него неудобным.
- Решение: группа «Документация ИОС» удалена из сайдбара, пункт вынесен
  как top-level sidebar-item. Страница page-docs-ios сохранена — её
  кнопка-вход (flowmeterMenuBtn) и breadcrumb-навигация работают как и
  раньше.
- Файлы изменены: index.html (сайдбар: -группа, +top-level пункт),
  sw.js (v503), tests/test-role-access.js (SIDEBAR_GROUPS и тесты
  групп), tests/test-flowmeter-validation.js (+6 тестов Task 239,
  Task 238 SW-блок → историческая заметка).
- Пользователю: после обновления PWA (v503) открыть сайдбар — раздел
  «Расходомеры хозрасчётные» должен быть виден сразу (для ролей
  КИП8 pro / КИП ИОС дежурный / ИТР8 / ИТР8 pro / ИТР ИОС / Админ).
  Клик по нему открывает страницу расходомеров напрямую (как и раньше
  через кнопку flowmeterMenuBtn на странице «Документация ИОС»).
