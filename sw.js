...
// Task 262: производственный календарь — основной источник legalic
// (официальные нормы, цепочки переносов, 2027 preliminary), резерв
// isDayOff; День шахтёра (Кузбасс) вычисляется автоматически.
...
// Task 269: окно календаря — нормы двумя столбиками, окно по ширине
// текста справа / кнопки слева внизу бара, единая высота кнопок,
// «Загрузка…» шрифтом расходомеров.
// Task 270: окно — высота под 5 строк (95px), светлая тема — фон окна
// темнее (#e9e7de), плашки-заголовки столбцов с цветным фоном и ярким
// текстом, отступы бара 5px (бар узкий, по высоте окна).
// Task 272: кнопка «Календарь» → «Обновить» (диалог с источником
// legalic и подтверждением), шторка настроек с выбором региона и
// токеном production-calendar.ru удалена (регион один — 42),
// «Сформировать» строит шахматку на ВЕСЬ ГОД (12 месяцев), кнопки
// перенесены в ЛЕВЫЙ ВЕРХНИЙ угол бара.
// Task 274: автоматическая расстановка отпусков в шахматке —
// (история задач 262–274 сокращена; полные записи — в worklog.md)
// Task 288: удалён блок «Передано показаний (контроль)» из карточки
// Хозрасчёта №1 вместе с логикой счёта (flowCountStats/.flow-stats-*);
// формы ввода за сутки/неделю/месяц и запись в архив — без изменений.
// Task 289: форма ввода «за неделю» убрана (остались сутки/месяц);
// счётчик «За неделю» (сумма суточных за пн–вс) — справа от заголовка
// «Хронология показаний»; дни периода в архиве — включительно.
// Task 296 (Этап 4): клиент переключён на серверную карту прав из матрицы
// KIP8_Access (getMyAccess): видимость разделов и кнопка «Ввести
...
// Task 307: в «Графике работы» удалена вкладка/страница «Сотрудники»;
// кнопка «+ Сотрудник» (добавление) — в тулбаре над шахматкой,
// после добавления перезагружается шахматка.
// Task 331: шторка «Итоги учёта» — без полосок вертикальной прокрутки
// (нативные полосы скрыты во всех движках, горизонтальные — КАСТОМНЫЕ
// ползунки .ws-grid-hbar/.ws-tt-hbar ПОД нижними бордюрами обеих
// колонок на одном уровне); шеврон вдвое уже и прозрачнее; левый
// бордюрчик — 2px полоса как разделитель сменных/дневных; шапка
// столбцов шторки = высоте шапки сетки (thead 37px, шапка шторки
// без филлера); «Явки (дни)»/«Переработка (дни)» — подписи в днях;
// текст шапки/значений по центру; нижние бордюры шторки и сетки
// ВСЕГДА на одном уровне (обе колонки: скролл-зона + 5px + 12px).
const CACHE_VERSION = 'kipia-test-v576';
